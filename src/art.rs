...
use crate::assets::ANSI;
use crate::assets::PRINT;
use crate::cli::Cli;
use crate::cli::Inject;
use crate::errors;
use crate::info::distro::DistroColors;
use crate::info::Info;
...
impl Art {
    pub fn new(info: &mut Info, args: &Cli) -> Self {
        ...
        // Get inner & distro colors.
        {
            match args.ascii_distro {
                ...
                Some(a) => {
                    let got = ascii_art::get(&a.to_string());
                    to_return.inner = String::from(got.0);
                    info.distro.colors = DistroColors::from(got.1);
                }
            }
